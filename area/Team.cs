...
public class Team {
    ...
    public void InitFormation(int position) {
        ...
        for (int i = 0; i < 4; i++) {    
            ...
            for (int j = 0; j < formations[i]; j++) {
                this.players[indexPlayer] = new BabyFootPlayer(((j + 1) * (babyFoot.GetWidthArea() / (formations[i] + 1))) - 20, (position == 0) ? y : (babyFoot.GetHeightArea() - 45) - y, babyFoot, owner, color, this);
                indexPlayer++;
            }
        }
    ...
    public BabyFootPlayer GetPlayersWithCurser() {
        return players[this.Position]; // Get the tige compared with position
    }

    public int GetPlayersLowDistanceWithBall() {
    ...
    public int GetPlayersLowDistanceWithCurser() {
    ...
    public void ChangeCurser() {
        this.Position = GetPlayersLowDistanceWithBall();
    ...
    public void Move(int rotation) {
        ...
        if (player.IsTouchBall()) {
            if (rotation == 0) {
                babyFoot.ball.MoveTo(this.goal);
            } else {
                babyFoot.ball.MoveTo(players[GetPlayersLowDistanceWithCurser()]);
            }
        }
    }
    ...
    public void reverseGoal() {
        ...
        if (goalkeeper.IsTouchBall() && goalkeeper.IsGoal) {
            babyFoot.ball.MoveTo(this.goal);
        }
    }
